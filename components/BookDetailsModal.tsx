...
export const BookDetailsModal = ({ book, onClose, onRent }: { book: Book | null, onClose: () => void, onRent: (book: Book) => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in" onClick={onClose}>
      <GlassCard className="w-full max-w-4xl max-h-[90vh] overflow-hidden p-0 bg-white flex flex-col md:flex-row relative shadow-2xl" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
        <button 
          onClick={onClose}
        ...
        {/* Image Section */}
        <div className="w-full md:w-5/12 bg-slate-100 relative shrink-0 flex items-center justify-center p-6 md:p-0">
          ...
          <div className="p-6 md:p-8 flex-1">
            <div className="mb-6">
               <div className="md:hidden mb-2">
                 <Badge color="bg-emerald-100 text-emerald-700">{localize(book, 'genre')}</Badge>
               </div>
              <h2 className="text-2xl md:text-3xl font-bold text-slate-800 mb-2 leading-tight">{localize(book, 'title')}</h2>
              <p className="text-lg text-emerald-600 font-medium">{localize(book, 'author')}</p>
            </div>

            <div className="flex flex-wrap gap-3 mb-8">
               <div className="flex-1 min-w-[100px] p-3 bg-slate-50 rounded-xl border border-slate-100 flex flex-col items-center justify-center text-center">
                  <span className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">{t('available')}</span>
                  <span className={`text-xl font-bold ${book.available_copies > 0 ? 'text-green-600' : 'text-red-500'}`}>
                    {book.available_copies}
                  </span>
               </div>
               <div className="flex-1 min-w-[100px] p-3 bg-slate-50 rounded-xl border border-slate-100 flex flex-col items-center justify-center text-center">
                  <span className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">{t('daily_rate')}</span>
                  <span className="text-xl font-bold text-emerald-600">
                    {t('currency')}{book.daily_rate}
                  </span>
               </div>
               <div className="flex-1 min-w-[100px] p-3 bg-slate-50 rounded-xl border border-slate-100 flex flex-col items-center justify-center text-center">
                  <span className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">{t('copies')}</span>
                  <span className="text-xl font-bold text-slate-700">{book.total_copies}</span>
               </div>
            </div>
